...
// 削除処理
    ...
    // UPDATE文
    $sql = 'UPDATE `words` SET `delete_flag`=1 WHERE `id`='.$_GET['id'];

    $stmt = $dbh->prepare($sql);
    $stmt->execute();

    header('Location: english_words.php');
    exit();
    }
    ...
    $sql = 'INSERT INTO `words` SET `word`=?,
                                    `mean`=?,
                                    `created`=NOW()';  //NOW関数と言って現時点の時間を自動に入れてくれる
    $data = array($_POST['word'], $_POST['mean']);
    $stmt = $dbh->prepare($sql);
    $stmt->execute($data);
    ...
    $sql = 'SELECT * FROM `words` WHERE `delete_flag`=0 ORDER BY `created` DESC';
    $stmt = $dbh->prepare($sql);
    $stmt->execute();
  ...
  <div class="container">
    ...
    <div class="row">
      ...
      <div class="col-md-4 content-margin-top">
        <!-- form部分 -->
        <form action="english_words.php" method="post">
          <!-- nickname -->
          <div class="form-group">
            <div class="input-group">
              <input type="text" name="nickname" class="form-control" id="validate-text" placeholder="word" required>
          ...
          <div class="form-group">
            <div class="input-group">
              <textarea type="text" class="form-control" name="comment" id="validate-length" placeholder="meaning" required></textarea>
              <span class="input-group-addon danger"><span class="glyphicon glyphicon-remove"></span></span>
            </div>
          </div>
          <!-- interested in -->
            ...
            <article class="timeline-entry">
                <div class="timeline-entry-inner">
                    ...
                    <div class="timeline-label">
                      <h2><a href="#">
                            <?php echo $posts[$i]['word']; ?></a><br>?>
                          <span><?php echo $posts[$i]['created']; ?></span></h2>
                        <p><?php echo $posts[$i]['mean']; ?></p>
                     <!-- localhost/oneline_bbs/bbs.php?type=delete&id=10←消したいid番号 -->
                     <!-- idをそれぞれに割り振りたい場合phpを入れ込む -->
                        <p><a href="english_words.php?type=delete&id=<?php echo $posts[$i]['id'] ?>"><i class="fa fa-trash" aria-hidden="true"></i></a></p>
                    </div>
                </div>
            </article>
          <?php } ?>
